...
arrConstruido = hora_minuto_full.map((item)=>{
    console.log(item.replace(reg_5,(match, p1, p2, offset, string)=>`${p2} ${p1}|`));
    return item.replace(reg_5,(match, p1, p2, offset, string)=>`${p2} | `)
})

console.log(arrConstruido);
for (let index = 0; index < arrConstruido.length; index++) {
    console.log(arrConstruido.splice(0,2).reverse().toString());
    arrConstruidoFinal.push(arrConstruido.splice(0,2).reverse().toString())
}
console.log(arrConstruidoFinal);
